...
AddHostWindow::AddHostWindow(Widget *parent): ContentWindow(parent, "Add Host") {
    ...
    auto bg_color = Color(150, 150, 150, 200);
    
    for (auto key: keys) {
        auto button = number_buttons_container->add<Button>(key);
        button->set_background_color(bg_color);
    ...
    dot->set_background_color(bg_color);
    ...
    backspace->set_background_color(bg_color);
    ...
    connect->set_background_color(bg_color);
