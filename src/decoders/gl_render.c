...
static void compile_program() {
    ...
    check_compile_status(vert);
    
    glShaderSource(frag, 1, &fragment_shader_string, 0);
    glCompileShader(frag);
    check_compile_status(frag);
...
#define USE_GL_RED 1
#if USE_GL_RED
#define GL_TEXTURE_FORMAT GL_RED
#else
#define GL_TEXTURE_FORMAT GL_LUMINANCE
#endif
...
void gl_render_setup(int width, int height) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_TEXTURE_FORMAT, i > 0 ? width / 2 : width, i > 0 ? height / 2 : height, 0, GL_TEXTURE_FORMAT, GL_UNSIGNED_BYTE, NULL);
    }
}

void gl_render_draw(char* image[3]) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        glBindTexture(GL_TEXTURE_2D, texture_id[i]);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, i > 0 ? gl_render_width / 2 : gl_render_width, i > 0 ? gl_render_height / 2 : gl_render_height, GL_TEXTURE_FORMAT, GL_UNSIGNED_BYTE, image[i]);
